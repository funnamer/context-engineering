...
from app.agent_types import AgentAction, AgentProtocolFailure, AgentProtocolResult, AgentProtocolSuccess
...
def build_openai_tools(
    ...
    直接来自 policy；对于 `read`、`grep` 这类真实运行时工具，则优先从
...
def decode_agent_tool_call(message: Any, policy: dict) -> AgentProtocolResult:
    """把原生 function call 消息解析成统一动作结构，并按 policy 校验。
    ...
    if not tool_calls:
        return _invalid("模型未返回 function call", raw_response)

    if len(tool_calls) != 1:
        return _invalid("模型一次只能返回一个 function call", raw_response)
    ...
    try:
        ...
        return _invalid(f"function call 参数 JSON 解析失败: {exc}", raw_response)

    if not isinstance(parameters, dict):
        return _invalid("function call 参数必须是 JSON 对象", raw_response)
    ...
    return validate_agent_payload(payload, policy, raw_response)


def validate_agent_payload(payload: Dict[str, Any], policy: dict, raw_response: str) -> AgentProtocolResult:
    """根据策略检查已解析出的动作载荷是否合法。
    ...
    if think and not isinstance(think, str):
        return _invalid("字段 'think' 必须是字符串", raw_response)

    if not isinstance(tool, str) or not tool.strip():
        return _invalid("字段 'tool' 必须是非空字符串", raw_response)

    if not isinstance(parameters, dict):
        return _invalid("字段 'parameters' 必须是对象", raw_response)

    allowed_actions = {action.name: action for action in policy["actions"]}
    ...
    if schema:
        ...
        if validation_error:
            return _invalid(f"动作 '{tool}' 参数不合法: {validation_error}", raw_response)

    return AgentProtocolSuccess(
        payload=AgentAction(
            think=think,
            tool=tool,
            parameters=parameters,
...
def _invalid(error: str, raw_response: str) -> AgentProtocolFailure:
    """构造统一的失败返回结构。

    不同校验分支都可能失败，如果每个地方都手写一遍返回字典，代码会
    比较散。抽成这个小函数后，所有失败路径都会返回一致的数据格式，
    上层处理起来也更稳定。
    """
    return AgentProtocolFailure(error=error, raw_response=raw_response)
